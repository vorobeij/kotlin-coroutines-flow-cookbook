...
fun main() {
    runSerial()
    //kotlins.runSerialRxJava()
    //kotlins.runConcurrent()
    //kotlins.runConcurrentRxJava()
    //kotlins.runWithoutCriticalProtection()
    //kotlins.runWithCriticalProtection()
    //kotlins.syncSomething()
    //Thread.sleep(10000)
}
...
private fun someAsyncTaskRxJava(iteration: Int): Single<Int> {
    return Single.timer(3000L / iteration, TimeUnit.MILLISECONDS).map {
        log("[RxJava] Executed iteration: $iteration")
        iteration
    }
}
...
/*
    ...
    private fun kotlins.runSerial() = runBlocking {
        kotlins.someAsyncTask(2)
        kotlins.someAsyncTask(1)
        kotlins.someAsyncTask(3)
    }
 */
...
private fun runSerialRxJava() {
    someAsyncTaskRxJava(2)
        .flatMap { someAsyncTaskRxJava(1) }
        .flatMap { someAsyncTaskRxJava(3) }
...
/*
    ...
    private fun kotlins.runConcurrent() = runBlocking {
        launch { kotlins.someAsyncTask(2) }
        launch { kotlins.someAsyncTask(1) }
        launch { kotlins.someAsyncTask(3) }
    }
 */
...
private suspend fun synchronizeSomething(value: Int): Deferred<Boolean> = coroutineScope {
    syncMutex.withLock {
        ...
        return@coroutineScope async {
            delay(1000)
            log("I return things nicely $value")
            syncDeferred = null
            true
        }.also {
            syncDeferred = it
...
/*
    ...
    Removing mutex from kotlins.synchronizeSomething will result in:
    I return things nicely 1
